...
/* ══════════════════════════════════════════════════════════════
   ...
   ▸ Hide-on-scroll-down / reveal-on-scroll-up with spring physics
...
export const MotionNavbar: React.FC = () => {
  ...
  const handleScroll = useCallback(() => {
    ...
    requestAnimationFrame(() => {
      ...
      // Direction detection with 8px dead zone
      ...
      // Only hide when scrolled past hero AND going down
      const shouldHide = scrollDirection.current === 'down' && currentY > 400;
  ...
  useEffect(() => {
    ...
    sectionIds.forEach((id, idx) => {
      ...
      const trigger = ScrollTrigger.create({
        ...
        onToggle: (self) => {
          if (self.isActive) {
            // Map section IDs to nav link indices
            const navIdx = id === 'hero' ? -1 :
              id === 'story-scene' ? 0 :
              id === 'selector' ? 1 :
              id === 'calculator' ? 2 : -1;
  ...
  useEffect(() => {
    if (!indicatorRef.current || activeSection < 0) {
      // Hide indicator when no section is active (hero/top)
  ...
  return (
    <header
      ...
      style={{
        ...
        transition: 'transform 0.45s cubic-bezier(0.25, 1, 0.5, 1), padding-top 0.5s ease-out',
      }}
    >
      <div
        ref={navBarRef}
            ...
            {NAV_LINKS.map((link, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Animated underline — only shows on hover when NOT active */}
                  {!isActive && (
