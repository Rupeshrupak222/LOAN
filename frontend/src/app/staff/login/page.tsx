...
// Internal operational staff demo accounts (isolated to internal staff login)
const STAFF_DEMO_ACCOUNTS: { role: RoleName; email: string }[] = [
...
export default function StaffLoginPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const authResult = await login(identifier, password);
      const isCustomer =
        authResult?.user?.roles?.includes('CUSTOMER') && authResult.user.roles.length === 1;
      if (isCustomer) {
  ...
  return (
    <div className="grid min-h-screen lg:grid-cols-2 bg-slate-50">
      {/* Brand Hero Panel (Left side) - Internal LMS Operations */}
        ...
        <div className="relative z-10 my-auto py-8 space-y-6 max-w-lg">
          <div>
            <span className="inline-flex items-center gap-1.5 rounded-full bg-brand-500/20 border border-brand-400/30 px-3 py-1 text-xs font-semibold text-brand-300">
              <Sparkles className="h-3.5 w-3.5 text-brand-400" /> Enterprise Lending Operations
            </span>
            ...
            <p className="mt-3 text-sm text-slate-300 leading-relaxed">
              Origination queues, underwriting workflows, risk engine governance, financial ledger reconciliation, and collection recovery.
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-slate-700/80 bg-slate-800/80 p-5 backdrop-blur-md shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-700/60 pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-xs font-bold text-white">Adyapan Lending OS</p>
                  <p className="text-[10px] text-slate-400">Institutional Workspace</p>
                </div>
              </div>
            ...
            <div className="flex items-center gap-2 pt-1 text-[11px] text-slate-300">
              ...
              <span>Role-based access control with granular audit logging</span>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-md animate-fade-in space-y-6">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-7 sm:p-8 shadow-card space-y-5">
            <div>
              ...
              <p className="mt-0.5 text-xs text-slate-500">Sign in with your corporate employee credentials</p>
            </div>
            ...
            {/* Development / Testing Quick-Fill Helper */}
