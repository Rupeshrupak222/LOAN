...
export default function MyCasesPage() {
  ...
  // Forward to Finance Officer Mutation
  const forwardToFinanceMutation = useMutation({
    mutationFn: async (appId: string) => {
      return api.post(`/underwriting/${appId}/forward-to-finance`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['underwriting-queue'] });
      queryClient.invalidateQueries({ queryKey: ['underwriter-dashboard-queue'] });
      ...
      toast.success('Sanction proposal successfully forwarded to Finance Officer for disbursement release.');
    },
    onError: (err) => {
      toast.error(apiErrorMessage(err), { title: 'Could not forward to Finance' });
    },
  });
                ...
                {filteredCases.map((item: any) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* 13. State-Aware Action */}
                      <td className="py-3 px-3 text-right whitespace-nowrap">
                        {isApproved ? (
                          !isReadyForDisbursement ? (
                            <Button
                              size="sm"
                              disabled={forwardToFinanceMutation.isPending}
                              onClick={() => forwardToFinanceMutation.mutate(item.id)}
                            ...
                            <span className="text-[11px] font-semibold text-emerald-600 dark:text-emerald-400 flex items-center justify-end gap-1">
                              <CheckCircle2 className="w-3.5 h-3.5" /> Forwarded to Finance
                            </span>
                          )
                        ) : isRejected ? (
