    ...
    <div className="space-y-6">
      ...
      <Card className="p-5 shadow-sm dark:shadow-xl">
        <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100 mb-1 flex items-center gap-2">
          <PieChart className="h-4 w-4 text-blue-500" /> Delinquency Migration & Roll-Forward Matrix
        </h3>
          ...
          <table className="w-full text-xs text-slate-700 dark:text-slate-300 text-left">
            <thead className="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/60 text-slate-500 dark:text-slate-400 uppercase">
              <tr>
                ...
                <th className="py-2.5 px-3 text-right text-emerald-600 dark:text-emerald-400">Roll-Back (Cured)</th>
                <th className="py-2.5 px-3 text-right text-rose-600 dark:text-rose-400">Roll-Forward (Worse)</th>
                <th className="py-2.5 px-3 text-right font-bold text-slate-900 dark:text-slate-100">Ending Volume</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
              {analytics?.rollForwardMatrix?.map((r: any) => (
                <tr key={r.bucket} className="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                  <td className="py-2.5 px-3 font-semibold text-slate-800 dark:text-slate-200">DPD {r.bucket}</td>
                  <td className="py-2.5 px-3 text-right font-mono text-slate-700 dark:text-slate-300">{r.beginningCount}</td>
                  <td className="py-2.5 px-3 text-right font-mono text-slate-700 dark:text-slate-300">₹{r.beginningAmount.toLocaleString()}</td>
      ...
      <Card className="p-5 shadow-sm dark:shadow-xl">
        <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100 mb-1 flex items-center gap-2">
          <Award className="h-4 w-4 text-amber-500" /> Collector Performance Scorecards
        </h3>
        <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
              {performance?.length === 0 ? (
                ...
                performance?.map((off: any) => (
                  <tr key={off.officerId} className="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                    <td className="py-2.5 px-3">
                      <div className="font-semibold text-slate-800 dark:text-slate-200">{off.officerName}</div>
                      <div className="text-[10px] text-slate-500 font-mono">{off.officerEmail}</div>
                    </td>
                    <td className="py-2.5 px-3 text-right font-mono text-slate-700 dark:text-slate-300">{off.assignedCasesCount}</td>
                    <td className="py-2.5 px-3 text-right font-mono text-blue-600 dark:text-blue-400 font-semibold">{off.contactRatePct}%</td>
                    <td className="py-2.5 px-3 text-right font-mono text-slate-700 dark:text-slate-300">{off.ptpCreatedCount}</td>
                    <td className="py-2.5 px-3 text-right font-mono text-emerald-600 dark:text-emerald-400 font-semibold">{off.ptpKeptCount}</td>
                    <td className="py-2.5 px-3 text-right font-mono font-bold text-slate-900 dark:text-slate-100">{off.ptpFulfillmentRatePct}%</td>
