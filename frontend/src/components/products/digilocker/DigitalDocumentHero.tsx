...
export const DigitalDocumentHero: React.FC = () => {
  ...
  // Scroll separation and headline transition
  useEffect(() => {
    if (!containerRef.current || !transitionZoneRef.current) return;

    const ctx = gsap.context(() => {
      ScrollTrigger.create({
        trigger: transitionZoneRef.current,
        start: 'top 80%',
        end: 'bottom 20%',
        scrub: true,
        onUpdate: (self) => {
  ...
  return (
    <section
      ...
      className="relative min-h-[96vh] flex flex-col justify-between pt-24 sm:pt-28 pb-12 px-4 sm:px-8 lg:px-12 bg-[#FFFFFF] text-[#071A33] overflow-hidden border-b border-slate-200 select-none"
    ...
    >
      ...
      {/* ── Main Editorial Hero Split (55-60% Text / 40-45% 3D Document) ── */}
      <div className="max-w-[1400px] mx-auto w-full relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center my-auto py-8">
        {/* ── Left Column (58%): Monumental Editorial Typography ── */}
        <div className="lg:col-span-7 space-y-6 text-left">
          ...
          <h1
            ref={headlineRef}
            className="text-4xl sm:text-6xl lg:text-[68px] font-black text-[#071A33] tracking-tight leading-[1.02] uppercase font-sans"
          >
            VERIFY IDENTITY.{' '}
        ...
        {/* ── Right Column (42%): 3D Abstract Digital Document ── */}
      ...
      {/* ── Transition Zone: Scroll-Separating 3-Layer Narrative ── */}
      <div
        ref={transitionZoneRef}
        className="max-w-[1400px] mx-auto w-full pt-8 pb-4 text-center border-t border-slate-200/60"
      >
        <div className="flex flex-col items-center justify-center space-y-2">
          <p className="text-xs font-mono font-bold text-slate-400 uppercase tracking-widest">
            {scrollProgress > 0.3 ? 'ONE DOCUMENT. A CLEARER VERIFICATION JOURNEY.' : 'SCROLL TO EXPLORE THE WORKFLOW'}
          </p>
          <ChevronDown
            className={`w-4 h-4 text-[#155EEF] transition-transform duration-300 ${
              scrollProgress > 0.3 ? 'rotate-180 text-emerald-600' : 'animate-bounce'
            }`}
          />
        </div>
      </div>
